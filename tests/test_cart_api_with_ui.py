...
@allure.title("Добавление нескольких штук одного товара в корзину ")
def test_add_some_product_in_cart():
    ...
    with step('Добавить несколько товар в корзину через API'):
        response = requests.post(
            url= URL + 'addproducttocart/details/31/1',
    ...
    with step("Проверяем что товары в корзине"):
        browser.element('.product-name').should(have.exact_text('14.1-inch Laptop'))
...
@allure.title("При выполнении хапроса на добавление продукта в корзину с EnteredQuantity = 0 корзина пустая")
def test_empty_cart():
    with step("Авторизация через API"):
        result = requests.post(
        url= URL + "/login",
